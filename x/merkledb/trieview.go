...
func (t *trieView) visitPathToKey(key Key, visitNode func(*node) error) error {
	...
	for currentNode.key.bitLength < key.bitLength {
		...
		currentNode, err = t.getNodeWithID(nextChildEntry.id, key.Take(t.tokenConfig, t.tokenConfig.TokenLength(currentNode.key)+1+t.tokenConfig.TokenLength(nextChildEntry.compressedKey)), nextChildEntry.hasValue)
		if err != nil {
			return err
		}
		if err := visitNode(currentNode); err != nil {
