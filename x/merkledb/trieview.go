...
func (t *trieView) calculateNodeIDsHelper(n *node) ids.ID {
	...
	for updatedChild := range updatedChildren {
		index := updatedChild.n.key.Token(t.tokenConfig, keyLength)
...
func (t *trieView) compressNodePath(parent, node *node) error {
	...
	for index, entry := range node.children {
		childKey = node.key.AppendExtend(t.tokenConfig, index, entry.compressedKey)
		childEntry = entry
	}

	// [node] is the first node with multiple children.
	// combine it with the [node] passed in.
	parent.setChildEntry(childKey.Token(t.tokenConfig, t.tokenConfig.TokenLength(parent.key)),
		child{
			compressedKey: childKey.Skip(t.tokenConfig, t.tokenConfig.TokenLength(parent.key)+1),
			id:            childEntry.id,
...
func (t *trieView) visitPathToKey(key Key, visitNode func(*node) error) error {
	...
	for currentNode.key.bitLength < key.bitLength {
		// confirm that a child exists and grab its ID before attempting to load it
		nextChildEntry, hasChild := currentNode.children[key.Token(t.tokenConfig, t.tokenConfig.TokenLength(currentNode.key))]

		if !hasChild || !key.iteratedHasPrefix(t.tokenConfig, t.tokenConfig.TokenLength(currentNode.key)+1, nextChildEntry.compressedKey) {
			// there was no child along the path or the child that was there doesn't match the remaining path
			return nil
		}

		// grab the next node along the path
		currentNode, err = t.getNode(key.Take(t.tokenConfig, t.tokenConfig.TokenLength(currentNode.key)+1+t.tokenConfig.TokenLength(nextChildEntry.compressedKey)), nextChildEntry.hasValue)
		if err != nil {
			return err
		}
		if err := visitNode(currentNode); err != nil {
