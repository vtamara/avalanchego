...
func (m *manager) createChain(chainParams ChainParameters) {
	...
	m.Log.Info("building chain", zap.Stringer("chainID", chainParams.ID))
	chain, err := m.buildChain(chainParams, sb)
	m.Log.Info("buildChain returned", zap.Error(err))
	if err != nil {
...
func (m *manager) buildChain(chainParams ChainParameters, sb subnets.Subnet) (*chain, error) {
	...
	// Create the chain
	chainLog.Info("calling vmFactory.New")
	vm, err := vmFactory.New(chainLog)
	chainLog.Info("vmFactory.New return error", zap.Error(err))
	if err != nil {
	...
	for i, fxID := range chainParams.FxIDs {
		...
		chainLog.Info("calling fxFactory.New")
		fx, err := fxFactory.New(chainLog)
		chainLog.Info("fxFactory.New return error", zap.Error(err))
		if err != nil {
