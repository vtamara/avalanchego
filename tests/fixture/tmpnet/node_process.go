...
// Retrieve the name of the directory that the network of the node is stored
// in. This should be the uuid that uniquely identifies the temporary network.
func (p *NodeProcess) getNetworkUUID() (string, error) {
	networkDir, err := filepath.Abs(filepath.Join(p.node.getDataDir(), ".."))
	if err != nil {
		return "", fmt.Errorf("failed to retrieve network dir: %w", err)
	}
	return filepath.Base(networkDir), nil
}

// Return the path for this node's prometheus configuration.
func (p *NodeProcess) getPrometheusConfigPath(promDir string, networkUUID string) string {
	// Ensure a unique filename to allow config files to be added and removed
	// by multiple nodes without conflict.
	return filepath.Join(promDir, fmt.Sprintf("%s_%s.yaml", networkUUID, p.node.NodeID))
}
...
func (p *NodeProcess) removePrometheusConfig() error {
	...
	networkUUID, err := p.getNetworkUUID()
	if err != nil {
		return err
	}
	configPath := p.getPrometheusConfigPath(promDir, networkUUID)
...
func (p *NodeProcess) writePrometheusConfig() error {
	...
	promConfig := fmt.Sprintf(prometheusTemplate, nodeAddress, networkUUID, p.node.NodeID, p.node.IsEphemeral)
